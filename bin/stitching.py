...
def main():
    ...
    if not all(matches):
        ...
        if not isinstance(cr, int):
            ...
            reconstructed_image = image_reconstruction_loop(
                crops_files, shape, args.overlap_size
            )

            moving_channels = (
                os.path.basename(args.moving)
                .replace("padded_", "")
                .split(".")[0]
                .split("_")[2:][::-1]
            )  # Select first two channels (omit DAPI) and reverse list

            fixed_channels = (
                os.path.basename(args.fixed)
                .replace("padded_", "")
                .split(".")[0]
                .split("_")[1:][::-1]
            )  # Select all channels and reverse list

            moving_channels_to_export = remove_lowercase_channels(moving_channels)
            moving_channels_to_export_no_dapi = [
                ch for ch in moving_channels_to_export if ch != "DAPI"
            ]
            ...
            for idx, ch in enumerate(moving_channels_to_export_no_dapi):
                save_h5(
                    np.expand_dims(reconstructed_image[:, :, idx], axis=0).astype(
                        np.float32
                    ),
                    f"registered_{args.patient_id}_{ch}.h5",
                )
                save_tiff(
                    reconstructed_image[:, :, idx],
            ...
            for idx, ch in enumerate(fixed_channels_to_export):
                ...
                save_h5(image, f"registered_{args.patient_id}_{ch}.h5")

    else:
        tiff.imwrite(f"registered_{args.patient_id}.tiff", 0)

        save_h5(0, f"registered_{args.patient_id}.h5")
