...
def upscale_results(marker):
    if pipex_scale_factor > 0:
        ...
        image = tiff.imread(os.path.join(data_folder, "preprocessed", marker + ".tif"))
        ...
        image = resize(image, image.shape[0] * pipex_scale_factor, image.shape[1] * pipex_scale_factor, order=0, preserve_range=True, anti_aliasing=False).astype('uint16')
        #image = resize(np_img, (len(np_img) / pipex_scale_factor, len(np_img[0]) / pipex_scale_factor), order=0, preserve_range=True, anti_aliasing=False).astype('uint16')
        #image = np.array(image)
        tiff.imwrite(image, os.path.join(data_folder, "preprocessed", marker + ".tif"))
...
def preprocess_image(marker, marker_img):
    ...
    tiff.imwrite(os.path.join(data_folder, "preprocessed", marker + ".tif"), np.uint16(np_img * 65535))
    ...
    for file in os.listdir(data_folder):
        ...
        if next_try:
            try:
                for marker in preprocess_markers:
                    if marker + '.' in file:
                        curr_image = tiff.imread(file_path)
