...
def main():
    ...
    moving_channels = (
        os.path.basename(args.moving_image).split(".")[0].split("_")[2:][::-1]
    )
    ...
    current_channels_to_register_no_dapi = [
        ch for ch in current_channels_to_register if ch != "DAPI"
    ]
    ...
    if current_channels_to_register_no_dapi:
        if any(
            [
                e
                for e in current_channels_to_register_no_dapi
                if e in channels_to_register
            ]
        ):
            ...
            if len(np.unique(moving)) != 1 and len(np.unique(fixed)) != 1:
                ...
                logger.debug(
                    f"MOVING STACKED IMAGE SHAPE: {moving_channels_images.shape}"
                )

                logger.debug(f"Saving empty crop (unregistered): {args.crop_image}")
                save_h5(moving_channels_images, output_path)
                save_h5(np.squeeze(moving[:, :, -1]), output_path_dapi)
        else:
            save_h5(0, f"registered_0_0_{patient_id}.h5")
            save_h5(0, f"qc_0_0_{patient_id}.h5")
    else:
        ...
        if not os.path.exists(f"registered_{patient_id}_{random_hash}.h5"):
            save_h5(0, f"registered_{patient_id}_{random_hash}.h5")
            save_h5(0, f"qc_{patient_id}_{random_hash}.h5")
